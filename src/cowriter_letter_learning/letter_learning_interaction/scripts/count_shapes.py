...
class ShapeCounter:
    ...
    def on_word(self, message):
        ...
        if message.data != "end":
            ...
            print(
                "Number of corrections received for previous word: "
                + f" {self.number_of_corrections_received_this_word}"
            )
            print(
                "Number of corrections responded to for previous word: "
        ...
        print(
            "-------------------Number of word requests received: "
    ...
    def on_stop_request_received(self, message):
        ...
        print(
            "Total number of robot-drawn shape messages:"
            + f"{self.number_robot_shapes}"
        )
        print(
            "Total number of corrections ignored: "
        ...
        self.writer.writerow(
            ["Total number of words", str(self.number_of_words)]
        )
        self.writer.writerow(
            [
                "Number of words before test",
                str(self.number_of_words_before_test),
            ]
        )
        self.writer.writerow(
            [
                "Total number of robot-drawn letters",
                str(self.number_robot_shapes + 2 * self.number_of_words),
            ]
        )
        self.writer.writerow(
            [
                "Total number of user-drawn shapes",
                str(self.number_of_user_shapes),
            ]
        )
        self.writer.writerow(
            [
                "Total number of user-drawn shapes responded to",
                str(
                    self.number_of_user_shapes
                    - self.number_of_corrections_ignored
                ),
            ]
        )
...
if __name__ == "__main__":
    ...
    with open(args.output, "w") as csvfile, open(
        args.output_letters, "w"
    ) as csvfile_letters:
        writer = csv.writer(csvfile, delimiter=" ", quoting=csv.QUOTE_MINIMAL)

        writer_letters = csv.writer(
            csvfile_letters, delimiter=" ", quoting=csv.QUOTE_MINIMAL
        )
