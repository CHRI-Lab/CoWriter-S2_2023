...
learning_modes: Dict[str, LearningModes] = {'c': LearningModes.starts_random,
...
class InteractionSettings():
    ...
    @staticmethod
    def get_phrases(language: str) -> Tuple[str, str, str, List[str], List[str], List[str], List[str], List[str]]:
        """
        ...
        if language.lower() == 'english':
            intro_phrase: str = "Hello. I'm Nao. Please show me a word to practice."
            test_phrase: str = "Ok, test time. I'll try my best."
            thank_you_phrase: str = 'Thank you for your help.'

            demo_response_phrases: List[str] = ["Okay, I'll try it like you", "So that's how you write %s",
                                                "That's a much better %s than mine", "I'll try to copy you", "Let me try now", "Thank you"]
            asking_phrases_after_feedback: List[str] = ["Any better?", "How about now?", "Now what do you think?",
                                                        "Is there a difference?", "Is this one okay?", "Will you show me how?", "Did I improve?"]
            asking_phrases_after_word: List[str] = [
                "Okay, what do you think?", "This is a hard word", "Is this how you write it?", "Please help me"]
            word_response_phrases: List[str] = ["%s, okay. ", "%s seems like a good word",
                                                "Hopefully I can do well with this word", "%s. Let's try", "Okay, %s now"]
            word_again_response_phrases: List[str] = [
                "%s again, okay.", "I thought I already did %s", "You like to practice this word"]
            ...
            demo_response_phrases: List[str] = ["Ok", "D'accord, j'essaye comme ça", "Ah, c'est comme ça qu'on écrit %s", "Bon",
                                                "Ce %s est pas mal", "Bon, j'essaye comme toi", "D'accord", "Ok, à moi", "À mon tour", "Ok", "Merci, je vais essayer"]
            asking_phrases_after_feedback: List[str] = ["C'est mieux ?", "Voilà", "Et comme ça ?", "Tu en penses quoi ?", "Alors ?",
                                                        "Qu'est-ce que tu en penses ?", "Il y a une différence ?", "Ça va cette fois ?", "Je me suis amélioré ?", "Tu trouves que c'est mieux ?"]
            asking_phrases_after_word: List[str] = ["Bon, qu'est ce que tu en penses ?", "Pas facile !",
                                                    "Voilà", "C'est bien comme ça ?", "Bon", "Je crois que j'ai besoin d'aide.", "Et voilà !"]
            word_response_phrases: List[str] = [
                "D'accord pour %s", "Ok, j'essaye %s", "Bon, je devrais y arriver", "D'accord", "%s ? ok"]
            word_again_response_phrases: List[str] = [
                "Encore %s ? bon, d'accord.", "Je crois que j'ai déjà fait %s", "On dirait que tu aimes bien %s !", "Encore ?"]

        else:
            raise RuntimeError(f'Requested language ({language}) not supported')

        return (intro_phrase, demo_response_phrases, asking_phrases_after_feedback, asking_phrases_after_word,  # type: ignore
                word_response_phrases, word_again_response_phrases, test_phrase, thank_you_phrase)  # type: ignore
    ...
    def generate_settings(shape_type: str) -> settings_struct:  # type: ignore
        ...
        if InteractionSettings.dataset_directory is None:
            raise RuntimeError(
                "Dataset directory has not been set yet with set_dataset_directory()")
        ...
        dataset_file: str = InteractionSettings.dataset_directory + '/' + shape_type + '.dat'
        if not os.path.exists(dataset_file):
            raise RuntimeError("Dataset is not known for shape " + shape_type)

        # Load dataset parameter file
        dataset_param: str = InteractionSettings.dataset_directory + '/params.dat'
        if not os.path.exists(dataset_param):
            ...
            with open(dataset_param, 'r') as f:
                ...
                for line in f.readline():
                    # Find the line for the given shape type and get its parameters
