...
This module contains the ShapeDisplayManager class, which is
responsible for managing the positioning of shapes in a grid-like
structure. It assigns new shapes to their appropriate location and can
convert the location of an event into the shape which it was done
around.
...
class ShapeDisplayManager:
    ...
    def closest_shapes_to_location(self, location: List[float]) -> Tuple[List[int], List[int]]:
        ...
        Tuple[List[int], List[int]]
            ...
            shape type codes of the closest shapes, and the second
            list contains the shape IDs of the closest shapes.
        """
        [row, column] = self.index_of_location(location)
