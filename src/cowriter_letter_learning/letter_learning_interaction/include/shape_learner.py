...
"""
# flake8: noqa # line length for comments
...
)  # How different two shapes' parameters need to be to be published for comparison
...
class ShapeLearner:
    def __init__(self, settings):
        """
        Initializes an instance of the ShapeLearner class.

        Args:
        - settings: An object containing various settings and parameters.

        Attributes:
        - params_to_vary: A list of parameters to vary during shape learning.
        - num_principle_components: The number of principle components.
        - shape_modeler: An instance of the ShapeModeler class used for shape modeling.
        - bounds: The initial bounds for the parameters.
        - do_groupwise_comparison: A flag indicating whether to perform groupwise comparison.
        - shape_learning: A flag indicating the type of shape learning algorithm.
        - min_param_diff: The minimum difference allowed between parameter values.
        - initial_param_value: The initial parameter values.
        - params: An array storing the learned parameter values.
        - initial_bounds: The initial bounds for the parameters (copy of the original bounds).
        - converged: A flag indicating whether the shape learning has converged.
        - num_iters: The number of iterations performed during shape learning.
        - num_iters_converged: The number of iterations since convergence.
        """
        ...
        self.shape_modeler = ShapeModeler(
            init_filename=settings.init_dataset_file,
        ...
        for i in range(len(self.params_to_vary)):
            ...
            if numpy.isnan(settings.initial_bounds[i, 0]) or numpy.isnan(
                settings.initial_bounds[i, 1]
            ):  # want to set initial bounds as std. dev. multiple
                bounds_from_std_dev_multiples = (
                    numpy.array(settings.initial_bounds_std_dev_multiples[i, :])
    ...
    def start_learning(self):
        ...
        if numpy.isnan(self.initial_param_value[0]):
            [
                shape,
                paramValues,
            ] = self.shape_modeler.make_random_shape_from_uniform(
                self.params, self.params_to_vary, self.bounds
            )
        ...
        self.best_param_values = self.params[
            self.params_to_vary[0] - 1
        ]  # USE ONLY FIRST PARAM IN LIST FOR SELF-LEARNING ALGORITHM
    ...
    def start_learning_at(self, starting_bounds, starting_param_values):
        """
        Starts the learning process at a specific point.

        Args:
        - starting_bounds: The starting parameter bounds.
        - starting_param_values: The starting parameter values.

        Returns:
        - shape: The initial shape.
        - best_param_values: The initial parameter values.
        ...
        [
            shape,
            param_values,
        ] = self.shape_modeler.make_random_shape_from_traingular(
            self.params, self.params_to_vary, self.bounds, starting_param_values
        )
    ...
    def make_shape_different_to(self, param_value):
        """
        Creates a new shape that is different from the given parameter value.

        Args:
        - param_value: The parameter value to create a different shape from.

        Returns:
        - new_shape: The newly generated shape.
        - new_param_values: The corresponding parameter values of the new shape.
        """
        ...
        [
            ...
            self.params, self.params_to_vary, self.bounds, [param_value]
        ...
        num_attempts = 1
        while (
            abs(new_param_values - param_value) < self.min_param_diff
            ...
            new_param_values = new_params[
                self.params_to_vary[0] - 1, 0
            ]  # USE ONLY FIRST PARAM FOR SELF-LEARNING ALGORITHM ATM
            num_attempts += 1

        if (
            num_attempts >= MAX_NUM_ATTEMPTS
        ):  # couldn't find a 'different' shape in range
    ...
    def make_shape_similar_to(self, param_value):
        """
        Creates a new shape that is similar to the given parameter value.

        Args:
        - param_value: The parameter value to create a similar shape to.

        Returns:
        - new_shape: The newly generated shape.
        - new_param_value: The corresponding parameter value of the new shape.
        """
    ...
    def generate_simulated_feedback(self, shape, new_param_value):
        """
        Generates simulated feedback based on the given shape and new parameter value.

        Args:
        - shape: The shape for which feedback is generated.
        - new_param_value: The new parameter value.

        Returns:
        - best_shape_idx: The index of the best shape based on the simulated feedback.
        """
        ...
        if self.do_groupwise_comparison:
            ...
            errors = [
                abs(self.best_param_values - goal_param_value),
    ...
    def respond_to_feedback(self, best_shape):
        """
        Updates the best parameter values and bounds based on the feedback received.

        Args:
        - best_shape: The index of the best shape.
        ...
        if self.do_groupwise_comparison:
            ...
            best_param_value_index = (
                bisect.bisect(self.params_sorted, self.best_param_values) - 1
            )  # indexing seems to start at 1 with bisect
            new_bounds = [
            ...
            if (best_param_value_index + 1) < (
                len(self.params_sorted) - 1
            ):  # not the default max
    ...
    def generate_new_shape_given_feedback(self, best_shape):
        """
        Generates a new shape based on the feedback received.
        ...
        if (
            abs(self.bounds[0, 1] - self.best_param_values) - self.min_param_diff < TOL
        ) and (
            abs(self.best_param_values - self.bounds[0, 0]) - self.min_param_diff
        ) < TOL:  # USE ONLY FIRST PARAM FOR SELF-LEARNING ALGORITHM ATM
            self.converged = True
        else:
            self.converged = False
    ...
    def respond_to_demonstration(self, shape):
        ...
        return (
            self.shape_modeler.make_shape(self.params),
            self.params,
            params_demo,
        )
