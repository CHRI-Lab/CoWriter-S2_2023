...
class ShapedWord:
    ...
    def __init__(self, word: str, paths: List[List[Tuple[float, float]]], origin: Optional[List[float]] = None) -> None:
        """
        ...
        self.bounding_boxes: List[Tuple[int,
                                        int, int, int]] = self._compute_bbs()
        self.global_bounding_box: Tuple[float, float,
                                        float, float] = self._compute_global_bb()

        self.origin = origin if origin is not None else [0.0, 0.0]

    def get_letters_paths(self, absolute: Optional[bool] = None) -> List[List[Tuple[float, float]]]:
        """
    ...
    def get_letters_bounding_boxes(self, absolute: bool = True) -> Union[List[Tuple[float, float, float, float]], List[Tuple[int, int, int, int]]]:
        """
    ...
    def downsample(self, downsampling_factor: int) -> None:
        ...
        for path in self._paths:
            ...
            # Combine the x and y coordinates into a list of tuples representing the downsampled path
        ...
        # Recompute the bounding boxes of the letters and the global bounding box of the word
    ...
    def ispointonword(self, x: float, y: float) -> Tuple[bool, Optional[str], Optional[Tuple[float, float, float, float]]]:
        """
...
class TextShaper:
    ...
    @staticmethod
    def shape_word(word: ShapeLearnerManager, downsampling_factor: Optional[int] = None) -> ShapedWord:
        """
        ...
        for shape in word.shapes_of_current_collection():
            ...
            glyph = ShapeModeler.normalise_shape_height(
                shape.path)  # type: ignore
...
class ScreenManager:
    ...
    def place_reference_boundingboxes(self, word: str) -> List[Tuple[float, float, float, float]]:
        """
        ...
        self.ref_word = word
        self.ref_boundingboxes = [
            (x1 + origin[0], y1 + origin[1], x2 + origin[0], y2 + origin[1]) for x1, y1, x2, y2 in bbs]

        return self.ref_boundingboxes

    def closest_letter(self, x: float, y: float, strict: bool = False) -> Tuple[Optional[str], Optional[Tuple[float, float, float, float]]]:
        """
    ...
    def find_letter(self, path: List[Tuple[float, float]]) -> Tuple[Optional[str], Optional[Tuple[float, float, float, float]]]:
        """
    ...
    def split_path_from_template(self, path: List[Tuple[float, float]]) -> Dict[str, List[Tuple[float, float]]]:
        """
    ...
    @staticmethod
    def intersect(bb1: Tuple[float, float, float, float], bb2: Tuple[float, float, float, float]) -> bool:
        """
