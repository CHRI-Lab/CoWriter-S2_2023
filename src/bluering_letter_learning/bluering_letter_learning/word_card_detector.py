...
def last_seen_since(frame):
    try:
        return (
            rospy.Time().now()
            - tf_listener.getLatestCommonTime(CAMERA_FRAME, frame)
        ).to_sec()
    except tf.Exception:
        # card *never seen*
        return 10000
...
if __name__ == "__main__":
    ...
    prevWord = ""
    wordToPublish = ""
    go_card_last_seen = 0.0  # in seconds
    ...
    while not rospy.is_shutdown():
        ...
        for tag, letter in tags_letters_mapping.items():
            try:
                ...
                if (rospy.Time().now() - t).to_sec() < 0.3:
                    ...
                    lettersDetected.add(
                        (letter, trans[0])
                    )  # trans[0] -> x coord in camera frame
        ...
        if not lettersDetected:
            ...
            if wordToPublish in prevWord:
                rospy.loginfo(
                    "I'm not publishing '%s' since it is still the same word (of part thereof)."
                    % wordToPublish
                )
            else:
