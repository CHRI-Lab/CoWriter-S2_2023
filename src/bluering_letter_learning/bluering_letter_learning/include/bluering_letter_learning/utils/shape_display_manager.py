...
positionList_shape0 = [[1, 1], [0, 0], [2, 0], [1, 0], [0, 1], [2, 1], [
    0, 2], [2, 2], [1, 2], [0, 3], [2, 3], [1, 3], [1, 4], [0, 4], [2, 4]]
positionList_shape1 = [[1, 2], [0, 2], [2, 2], [1, 1], [1, 3], [0, 1], [
    0, 3], [2, 1], [2, 3], [1, 0], [0, 0], [2, 0], [1, 4], [0, 4], [2, 4]]
positionList_shape2 = [[1, 3], [0, 4], [2, 4], [1, 4], [1, 2], [0, 3], [
    2, 3], [0, 2], [2, 2], [0, 1], [2, 1], [1, 1], [1, 0], [0, 0], [2, 0]]
...
class ShapeDisplayManager:  # TODO make implementation of abstract class/interface
    ...
    def displayNewShape(self, shapeType_code):
        if shapeType_code > len(positionList)-1:
            ...
            if foundSpace:
                shapeID = numpy.equal(
                    self.shapesDrawn[:, :, 0], shapeType_code).sum()
        ...
        if row > numRows - 1 or row < 0:
            ...
            for i in range(numShapesDrawn):
                distsToLocation[i] = numpy.sqrt(
                    (row-shapeIndexes[i, 0])**2 + (col-shapeIndexes[i, 1])**2)
            ...
            while (i < len(closest_indexes) and distsToLocation[closest_indexes[i]] == distsToLocation[closest_indexes[0]]):
