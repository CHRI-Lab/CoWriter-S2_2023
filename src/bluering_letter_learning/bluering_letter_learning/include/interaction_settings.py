...
"""
Manages the settings used for the interactive_learning.py node. 
"""
...
class InteractionSettings:
    ...
    def getTrajectoryTimings(naoWriting):
        if naoWriting:
            t0 = 3
            # ? Time allowed for the first point in traj (seconds) (@todo should be proportional to distance)
            dt = 0.25  # ? Seconds between points in traj
            delayBeforeExecuting = 3
            # ? How far in future to request the traj be executed (to account for transmission delays and preparedness)
        else:
            t0 = 0.01
            dt = 0.1
    ...
    def getHeadAngles():
        ...
        headAngles_lookAtPerson_right = [
            -0.9639739513397217,
            0.08125996589660645,
        ]
        headAngles_lookAtPerson_left = [0.9639739513397217, 0.08125996589660645]
        return {
            "headAngles_lookAtTablet_down": headAngles_lookAtTablet_down,
            "headAngles_lookAtTablet_right": headAngles_lookAtTablet_right,
            "headAngles_lookAtTablet_left": headAngles_lookAtTablet_left,
            "headAngles_lookAtPerson_front": headAngles_lookAtPerson_front,
            "headAngles_lookAtPerson_right": headAngles_lookAtPerson_right,
            "headAngles_lookAtPerson_left": headAngles_lookAtPerson_left,
        }
    ...
    def getPhrases(language):
        if language.lower() == "english":
            introPhrase = (
                "Hello. I'm Nao. Nice to meet you. Hope I can help you good."
            )
    ...
    def generateSettings(shapeType):
        print(f"[generateSettings] shapeType : {shapeType}")
        if datasetDirectory is None:
            raise RuntimeError(
                "Dataset directory has not been set yet with setDatasetDirectory()"
            )

        paramsToVary = [3]
        # ? Natural number between 1 and numPrincipleComponents, representing which principle component to vary from the template
        initialBounds_stdDevMultiples = numpy.array(
            [[-6, 6]]
        )  # ? Starting bounds for paramToVary, as multiples of the parameter's observed standard deviation in the dataset
        doGroupwiseComparison = (
            True  # ? instead of pairwise comparison with most recent two shapes
        )
        initialParamValue = 0.0
        initialBounds = numpy.array([[numpy.NaN, numpy.NaN]])

        datasetFile = datasetDirectory + "/" + shapeType + ".dat"
        print(f"[generateSettings] datasetFile : {datasetFile}")
        ...
        print(f"[generateSettings] datasetParam : {datasetParam}")
        if not os.path.exists(datasetParam):
            ...
            with open(datasetParam, "r") as f:
                initialParamValue = [
                    0.0
                ] * 20  # HACK: this (default) value should be independant from the number of params. Smthg like np.NaN would be better
                for line in f.readline():
                    if (
                        line[1:-2] == shapeType
                    ):  # ? lines starting with [<letter>]\n
                        initialParamValue = [
