...
from bluering_letter_learning.controllers.ros_callback_controller import ROSCbController
...
class StateController(ROSCbController):
    ...
    def __init__(self,
                 ...
                 pub_camera_status, pub_bounding_boxes, pub_clear, tabletWatchdog,
                 pub_traj, pub_traj_downsampled,
                 pub_listening_signal
                ) -> None:

        super().__init__(stateMachine, naoIP, naoPort, naoConnected, effector, naoWriting, naoSpeaking, naoStanding, language, isFrontInteraction, personSide, alternateSidesLookingAt, datasetDirectory, shapeLoggingPath, shapeHelperFrame, pub_camera_status, pub_bounding_boxes, pub_clear, pub_traj, pub_traj_downsampled, pub_listening_signal)

        rospy.loginfo(f'[StateController] init called')
        ...
        introPhrase, self.demo_response_phrases, self.asking_phrases_after_feedback, self.asking_phrases_after_word, self.word_response_phrases, self.word_again_response_phrases, self.testPhrase, self.thankYouPhrase = InteractionSettings.getPhrases(language)
    ...
    def respondToFeedback(self, infoFromPrevState):
        rospy.loginfo(f'[StateController][respondToFeedback] STATE: RESPONDING_TO_FEEDBACK')
        ...
        try:
            ...
            rospy.logerr(f'[respondToFeedback] Shape type index must be an integer. Received : {feedback[0]}')
            processMessage = False

        try:
            bestShape_index = int(feedback[1])
        except:
            rospy.logerr(f'[respondToFeedback] Best shape index must be an integer. Received : {feedback[0]}')
            processMessage = False
        ...
        if len(feedback) > 2:
            ...
            if feedbackMessage == 'noNewShape':
                ...
                processMessage = False
                rospy.logerr(f'[respondToFeedback] Unknown message received in feedback string: {feedbackMessage}')  

        if processMessage:
            if noNewShape: #? just respond to feedback, don't make new shape 
                if self.robotCtl is not None and self.naoSpeaking:
                    rospy.loginfo(f'[StateController][respondToFeedback] toSay: {toSay}')
                    toSay = 'Ok, thanks for helping me'
                    self.robotCtl.speak(toSay)

                #? pass feedback to shape manager
                response = self.wordManager.feedbackManager(shapeIndex_messageFor, bestShape_index, noNewShape)
                if response == -1:
                    rospy.logerr(f'[respondToFeedback] Something\'s gone wrong in the feedback manager')

            else:
                if self.naoSpeaking:
                    rospy.loginfo(f'[StateController][respondToFeedback] NAO: {toSay}')
                    shape_messageFor = self.wordManager.shapeAtIndexInCurrentCollection(shapeIndex_messageFor)
                    toSay = f'Ok, I\'ll work on the {shape_messageFor}'
                    self.robotCtl.speak(toSay)

                [numItersConverged, newShape] = self.wordManager.feedbackManager(shapeIndex_messageFor, bestShape_index, noNewShape)
    ...
    def startInteraction(self, infoFromPrevState):
        rospy.loginfo(f'[StateController][startInteraction] STATE: STARTING_INTERACTION')
        # if self.naoSpeaking:
    ...
    def waitForChild(self, infoFromPrevState):
        if infoFromPrevState['state_cameFrom'] != 'WAITING_FOR_CHILD':
            rospy.loginfo(f'[StateController][waitForChild] STATE: WAITING_FOR_CHILD')
            self.pub_camera_status.publish(True) #? turn camera on
        if infoFromPrevState['state_cameFrom'] == 'STARTING_INTERACTION':
            rospy.sleep(1)
            self.robotCtl.say("Okay Minh. Please log or register a child so we can start new session", self.personSide)
            self.pub_listening_signal.publish('convo') #? capturing audio input to listen for word to write
            pass
    ...
    def waitForRobotToConnect(self, infoFromPrevState):
        ...
        if infoFromPrevState['state_cameFrom'] != 'WAITING_FOR_ROBOT_TO_CONNECT':
            rospy.loginfo(f'[StateController][waitForRobotToConnect] STATE: WAITING_FOR_ROBOT_TO_CONNECT')
    ...
    def waitForShapeToFinish(self, infoFromPrevState):
        ...
        if infoFromPrevState['state_cameFrom'] != 'WAITING_FOR_LETTER_TO_FINISH':
            rospy.loginfo(f'[StateController][waitForShapeToFinish] STATE: WAITING_FOR_LETTER_TO_FINISH')
        ...
        if self.shapeFinished:
            ...
            ref_boundingboxes = self.screenManager.place_reference_boundingboxes(self.wordManager.currentCollection)
            for bb in ref_boundingboxes:
                boxesToPub = self.shapeHelper.make_bounding_box_msg(bb, selected=False)
                rospy.loginfo(f'[StateController][waitForShapeToFinish] called make_bounding_box_msg to calc boxesToPub = {len(bb)}')
            ...
            rospy.loginfo(f'[StateController][waitForShapeToFinish] infoForNextState = {infoForNextState}')
            try:
    ...
    def respondToNewWord(self, infoFromPrevState):
        rospy.loginfo(f'[StateController][respondToNewWord] STATE: RESPONDING_TO_NEW_WORD')
        wordToLearn = infoFromPrevState['wordReceived']
        wordSeenBefore = self.wordManager.newCollection(wordToLearn)
        if self.naoSpeaking:
            if wordSeenBefore:
                try:
                    toSay = self.word_again_response_phrases[self.word_again_response_phrases_counter] % wordToLearn
                except TypeError: #? string wasn't meant to be formatted
                    toSay = self.word_again_response_phrases[self.word_again_response_phrases_counter]
                self.word_again_response_phrases_counter += 1
                if self.word_again_response_phrases_counter == len(self.word_again_response_phrases):
                    self.word_again_response_phrases_counter = 0

            else:
                try:
                    toSay = self.word_response_phrases[self.word_response_phrases_counter] % wordToLearn
                except TypeError: #? string wasn't meant to be formatted
                    toSay = self.word_response_phrases[self.word_response_phrases_counter]
                self.word_response_phrases_counter += 1
                if self.word_response_phrases_counter == len(self.word_response_phrases):
                    self.word_response_phrases_counter = 0

            rospy.loginfo(f'[StateController][respondToNewWord] NAO: {toSay}')
            self.robotCtl.speak(toSay)
    ...
    def askForFeedback(self, infoFromPrevState): 
        rospy.loginfo(f'[StateController][askForFeedback] STATE: ASKING_FOR_FEEDBACK')
        centre = infoFromPrevState['centre']
        rospy.loginfo(f'[StateController][askForFeedback] state_camefrom = ' + infoFromPrevState['state_cameFrom'])

        if infoFromPrevState['state_cameFrom'] == 'PUBLISHING_WORD':
            wordWritten = infoFromPrevState['wordWritten']
            rospy.loginfo(f'[StateController][askForFeedback] Asking for feedback on word {wordWritten}')
            if self.naoSpeaking:
                try:
                    toSay = self.asking_phrases_after_word[self.asking_phrases_after_word_counter] % wordWritten
                except TypeError: #? string wasn't meant to be formatted
                    toSay = self.asking_phrases_after_word[self.asking_phrases_after_word_counter]
                self.asking_phrases_after_word_counter += 1
                if self.asking_phrases_after_word_counter == len(self.asking_phrases_after_word):
                    self.asking_phrases_after_word_counter = 0

                if self.alternateSidesLookingAt:
                    self.robotCtl.lookAndAskForFeedback(toSay, self.nextSideToLookAt)
            ...
            rospy.loginfo(f'[StateController][askForFeedback] Asking for feedback on letter {shapeType}')
            
            if self.naoSpeaking:
                try:
                    toSay = self.asking_phrases_after_feedback[self.asking_phrases_after_feedback_counter] % shapeType
                except TypeError: #? string wasn't meant to be formatted
                    toSay = self.asking_phrases_after_feedback[self.asking_phrases_after_feedback_counter]
                self.asking_phrases_after_feedback_counter += 1
                if self.asking_phrases_after_feedback_counter == len(self.asking_phrases_after_feedback):
                    self.asking_phrases_after_feedback_counter = 0
    ...
    def respondToTestCard(self, infoFromPrevState):
        rospy.loginfo(f'[StateController][respondToTestCard] STATE: RESPONDING_TO_TEST_CARD')
        if self.naoSpeaking:
            self.robotCtl.speak(self.testPhrase)
            rospy.loginfo(f'[StateController]NAO: {self.testPhrase}')
        nextState = 'WAITING_FOR_WORD'
    ...
    def stopInteraction(self, infoFromPrevState):
        rospy.loginfo(f'[StateController][stopInteraction] STATE: STOPPING')
        if self.naoSpeaking:
            self.robotCtl.speak(self.thankYouPhrase)
        if self.naoConnected:
            self.robotCtl.motionProxy.wbEnableEffectorControl(self.effector, False)
    ...
    def waitForWord(self, infoFromPrevState):
        if infoFromPrevState['state_cameFrom'] != 'WAITING_FOR_WORD':
            rospy.loginfo(f'[StateController][waitForWord] STATE: WAITING_FOR_WORD')
            self.pub_camera_status.publish(True) #? turn camera on
    ...
    def waitForFeedback(self, infoFromPrevState):
        if infoFromPrevState['state_cameFrom'] != 'WAITING_FOR_FEEDBACK':
            rospy.loginfo(f'[StateController][waitForFeedback] STATE: WAITING_FOR_FEEDBACK')
            self.pub_camera_status.publish(True) #? turn camera on
        ...
        if 'state_goTo' in infoForNextState:
            ...
            rospy.loginfo(f'[StateController][waitForFeedback] state_goTo = {state_goTo}')
    ...
    def waitForTabletToConnect(self, infoFromPrevState):
        ...
        if infoFromPrevState['state_cameFrom'] != 'WAITING_FOR_TABLET_TO_CONNECT':
            ...
            rospy.loginfo(f'[StateController][waitForTabletToConnect] STATE: waiting_for_tablet_to_connect')
        ...
        if self.tabletWatchdog.isResponsive(): #? reconnection - send message to wherever it was going
    ...
    def writeWord(self, infoFromPrevState):
        ...
        """
        rospy.loginfo(f'[StateController][writeWord] STATE: PUBLISHING_WORD')
        rospy.loginfo(f'[StateController][writeWord] len(self.demoShapesReceived) = {len(self.demoShapesReceived)}')
        ...
        traj = self.shapeHelper.make_traj_msg(placedWord, float(self.dt)/self.downSampleFactor, log = True)
        ...
        downsampledTraj = self.shapeHelper.make_traj_msg(downsampledShapedWord, self.dt, log = True)
        ...
        self.pub_traj_downsampled.publish(downsampledTraj) #? for the robot to write
        ...
        self.robotCtl.followTrajectory(downsampledTraj) #? control the robot to write
    ...
    def respondToDemonstrationWithFullWord(self, infoFromPrevState):
        rospy.loginfo(f'[StateController][respondToDemonstrationWithFullWord] STATE: RESPONDING_TO_DEMONSTRATION_FULL_WORD')
        ...
        if self.naoSpeaking:
            ...
            if self.demo_response_phrases_counter == len(self.demo_response_phrases):
                self.demo_response_phrases_counter = 0
            self.robotCtl.speak(toSay)
            rospy.loginfo(f'[StateController][respondToDemonstrationWithFullWord] NAO: {toSay}')
        ...
        for shape in self.demoShapesReceived:
            ...
            rospy.logdebug(f'[StateController][respondToDemonstrationWithFullWord] Downsampling %s...' % shapeName)
            glyph = self.shapeHelper.downsampleShape(glyph)
            rospy.loginfo(f'[StateController][respondToDemonstrationWithFullWord] Downsampling of %s done. Demo received for %s' % (shapeName, shapeName))
            rospy.loginfo(f'[StateController][respondToDemonstrationWithFullWord] self.wordManager.currentCollection: {self.wordManager.currentCollection} | shapeName : {shapeName} | self.wordManager.shapesLearnt : {self.wordManager.shapesLearnt}')
            shapeIndex = None
            shapeIndex = self.wordManager.currentCollection.index(shapeName)
            rospy.loginfo(f'[StateController][respondToDemonstrationWithFullWord] >>>  shapeIndex: {shapeIndex} ')
        ...
        rospy.loginfo(f'[StateController][respondToDemonstrationWithFullWord] self.demoShapesReceived reset! self.demoShapesReceived = {self.demoShapesReceived}')
