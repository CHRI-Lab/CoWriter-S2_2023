...
from bluering_letter_learning.include.utils import motion
from bluering_letter_learning.include.utils.writing_helper import (
    scaleAndFlipPoints,
    gapBetweenPoints,
)
...
class RobotController:
    ...
    def init(self) -> None:
        ...
        self.motionProxy.wbEnableEffectorControl(
            self.effector, False
        )  # ? if robot has fallen it will have a hard time getting up if the effector is still trying to be kept in a particular position. So, at init state, set effector to False and make the robot stand
    ...
    def lookAndAskForFeedback(self, toSay, side) -> None:
        ...
        if self.naoWriting:  # ? if the robot can write
            ...
            self.motionProxy.angleInterpolationWithSpeed(
                self.effector, self.armJoints_standInit, 0.3
            )

        if self.isFrontInteraction:
            self.motionProxy.setAngles(
                ["HeadYaw", "HeadPitch"],
                self.__head_configs__["headAngles_lookAtPerson_front"],
                0.2,
            )
        else:
            if (
                side == "Right"
            ):  # ? the person is on our right, then tell the robot to look right
                self.motionProxy.setAngles(
                    ["HeadYaw", "HeadPitch"],
                    self.__head_configs__["headAngles_lookAtPerson_right"],
                    0.2,
                )
            else:  # ? the person is on our left, then tell the robot to look left
                self.motionProxy.setAngles(
                    ["HeadYaw", "HeadPitch"],
                    self.__head_configs__["headAngles_lookAtPerson_left"],
                    0.2,
                )
        self.speak(toSay)
    ...
    def lookAtTablet(self) -> None:
        ...
        if self.isFrontInteraction:
            ...
            self.motionProxy.setAngles(
                ["HeadYaw", "HeadPitch"],
                self.__head_configs__["headAngles_lookAtTablet_down"],
                0.2,
            )
            ...
            if self.effector == "RArm":  # ? tablet will be on our right
                ...
                self.motionProxy.setAngles(
                    ["HeadYaw", "HeadPitch"],
                    self.__head_configs__["headAngles_lookAtTablet_right"],
                    0.2,
                )
            else:
                self.ros_node.get_logger().info(
                ...
                self.motionProxy.setAngles(
                    ["HeadYaw", "HeadPitch"],
                    self.__head_configs__["headAngles_lookAtTablet_left"],
                    0.2,
                )
    ...
    def followTrajectory(self, traj) -> None:
        ...
        if not self.hasFallen:  # ? no harm in executing trajectory
            ...
            for i, trajp in enumerate(traj.poses):
                ...
                if (
                    points
                ):  # ? what does this check? points is true or points not null ?
                    ...
                    if (
                        gapBetweenPoints(
                            ...
                            [
                                target_robot.pose.position.y,
                                target_robot.pose.position.z,
                            ],
                        )
                        > 0.003
                    ):
            ...
            self.ros_node.get_clock().sleep_for(
                Duration(
                    seconds=traj.header.stamp - self.ros_node.get_clock().now()
                )
            )
            startTime = self.ros_node.get_clock().now()

            # ? send position to proxy
            points_scaled_flipped = scaleAndFlipPoints(points)
            points_scaled_flipped = [
                [float(v) for v in item] for item in points_scaled_flipped
            ]  # ? convert list of numpy.float64 to list of float
