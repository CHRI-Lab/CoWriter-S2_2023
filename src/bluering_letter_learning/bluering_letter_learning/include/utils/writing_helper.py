...
def scaleAndFlipPoints(matrix):
    ...
    # ? scale up y
    scaleFactorY = abs(max(rangeOfPossibleY) - min(rangeOfPossibleY)) / abs(
        float(max(vectY)) - float(min(vectY))
    )
    vectY = [i * scaleFactorY - 0.02 for i in vectY]
    # print(f'scaleFactorY = {scaleFactorY}')

    # ? scale up Z
    scaleFactorZ = abs(max(rangeOfPossibleZ) - min(rangeOfPossibleZ)) / abs(
        float(max(vectZ)) - float(min(vectZ))
    )
    ...
    vectX = [
        math.sqrt(-math.pow((z - z0), 2) - math.pow((vectY[i] - y0), 2) + r * r)
        for i, z in enumerate(vectZ)
    ]
