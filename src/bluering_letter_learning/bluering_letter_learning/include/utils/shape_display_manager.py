...
class ShapeDisplayManager:  # TODO make implementation of abstract class/interface
    def __init__(
        ...
        # ? 3rd dim: shapeType_code, ID
        self.shapesDrawn = numpy.ones((3, 5, 2)) * numpy.NaN
    ...
    def displayNewShape(self, shapeType_code):
        if shapeType_code > len(positionList) - 1:
            ...
            while (not foundSpace) and (
                positionList_index < len(positionList[shapeType_code])
            ):
                # ? check next position in position list for this shape
                [row_test, col_test] = positionList[shapeType_code][
                    positionList_index
                ]
            ...
            if foundSpace:
                shapeID = numpy.equal(
                    self.shapesDrawn[:, :, 0], shapeType_code
                ).sum()
    ...
    def closestShapesToLocation(self, location):
        ...
        if row > numRows - 1 or row < 0:
            ...
            shapeIndexes = numpy.argwhere(
                numpy.isfinite(self.shapesDrawn[:, :, 0])
            )
            numShapesDrawn = shapeIndexes.shape[0]
            distsToLocation = numpy.zeros(numShapesDrawn)
            for i in range(numShapesDrawn):
                distsToLocation[i] = numpy.sqrt(
                    (row - shapeIndexes[i, 0]) ** 2
                    + (col - shapeIndexes[i, 1]) ** 2
                )
            ...
            while (
                ...
                and distsToLocation[closest_indexes[i]]
                == distsToLocation[closest_indexes[0]]
            ):
                nextClosest_row = shapeIndexes[closest_indexes[i], 0]
                nextClosest_col = shapeIndexes[closest_indexes[i], 1]
                shapeType_code.append(
                    self.shapesDrawn[nextClosest_row, nextClosest_col, 0]
                )
                shapeID.append(
                    self.shapesDrawn[nextClosest_row, nextClosest_col, 1]
                )
                i += 1
