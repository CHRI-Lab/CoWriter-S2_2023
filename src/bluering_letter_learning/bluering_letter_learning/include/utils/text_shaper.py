...
class ShapedWord:
    ...
    def compute_boundingbox(path):
        # rospy.loginfo(f'[ShapedWord][compute_boundingbox] path = {path}')
        x_min = 2000
        y_min = 2000
        x_max = 0
        y_max = 0
    ...
    def downsample(self, downsampling_factor):
        # rospy.loginfo(f'[ShapedWord][downsample] self._paths : {self._paths}')
        downsampled_paths = []
        ...
        for path in self._paths:
            ...
            # rospy.loginfo(f'[ShapedWord][downsample]   [+] path : {path}')

            downsampled_paths.append(path)

        self._paths = downsampled_paths
        # rospy.loginfo(f'[ShapedWord][downsample][+] downsampled_paths : {downsampled_paths}')
...
class ScreenManager:
    ...
    def split_path_from_template(self, path):
        ...
        """
        # rospy.loginfo(f'[ShapedWord][split_path_from_template] path = {path}')
        ...
        glyph = []
        # rospy.loginfo(f'[ShapedWord][split_path_from_template] path = {path}')
        # rospy.loginfo(f'[ShapedWord][split_path_from_template] path list = {list(path)}')
