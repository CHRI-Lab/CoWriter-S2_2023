...
class Watchdog:
    ...
    def defaultHandler(self):
        # rospy.loginfo(
        #     "Haven't received a clear on '" + self.clearTopic + "' topic"
        # )
    ...
    def isResponsive(self):
        if self.running:
            ...
            raise RuntimeError(
                "responsiveness cannot be determined while the watchdog is not running"
            )
...
class WatchdogClearer:
    ...
    def clearWatchdog(self):
        ...
        self.timer = Timer(
            self.timeBetweenClears_sec, self.clearWatchdog
        )  # ? schedule next clear
        self.timer.start()
