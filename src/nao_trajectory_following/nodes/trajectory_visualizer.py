...
class TrajectoryVisualizer(Node):
    ...
    def __init__(self):        
        ...
        CLEAR_TOPIC = self.declare_parameter('~clear_surface_topic','clear_screen').value
        SHAPE_TOPIC = self.declare_parameter('~trajectory_nao_topic','write_traj').value
        MARKER_TOPIC = self.declare_parameter('~visualization_output_topic','visualization_markers').value
        
        self.FRAME = self.declare_parameter('~writing_surface_frame_id','writing_surface').value
    ...
    def on_traj(self, requested_traj):
        ...
        for i in range(len(requested_traj.poses)-1):
            ...
            self.get_clock().sleep_for(Duration(seconds=duration)) #wait until it's time to show the next point
