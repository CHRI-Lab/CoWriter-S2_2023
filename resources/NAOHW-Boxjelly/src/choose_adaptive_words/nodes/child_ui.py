...
from PyQt5.QtCore import QObject, QRect, Qt, QSize, QDate, QLine
...
from std_msgs.msg import String, UInt8, Int32MultiArray, MultiArrayDimension, MultiArrayLayout
import numpy as np
...
class Child_UI(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.drawing.setStyleSheet("QLabel"
                                "{"
        ...
        self.button_erase.setIconSize(QSize(ERASR_SIZE[0] - ERASR_OFFSET, ERASR_SIZE[1] - ERASR_OFFSET)) 
        self.button_erase.setIcon(QtGui.QIcon(choose_adaptive_words_path + "/design/assets/gomme.png"))
        ...
        self.button_feedback.setIconSize(QSize(ERASR_SIZE[0] - ERASR_OFFSET, ERASR_SIZE[1] - ERASR_OFFSET)) 
        self.button_feedback.setIcon(QtGui.QIcon(choose_adaptive_words_path + "/design/assets/send.svg"))
        ...
        # init subscribers
        rospy.Subscriber(TOPIC_SHAPES_TO_DRAW, Int32MultiArray, self.callback_words_to_write)
        rospy.Subscriber(TOPIC_MANAGER_ERASE, String, self.callback_manager_erase)

        # init publisher
        self.publish_user_drawn_shapes = rospy.Publisher(TOPIC_USER_DRAWN_SHAPES, Int32MultiArray, queue_size=10)
        ...
        for ps in self.child_point_lists + [self.child_point_list]:
            if len(ps) > 1:
                for i in range(1, len(ps)):
                    line_seg = QLine(ps[i-1][0], ps[i-1][1], ps[i][0], ps[i][1])
    ...
    def feedback_clicked(self):
        ...
        for l in self.child_point_lists:
            ...
            i += 1
        print(total_list)
        self.publish_user_drawn_shapes.publish(self.pack_writing_pts(total_list))

    def resizeEvent(self, event):
        if hasattr(self, "button_erase"):
            self.button_erase.setGeometry(self.width() - ERASR_SIZE[0] - ERASR_OFFSET, ERASR_OFFSET, ERASR_SIZE[0], ERASR_SIZE[1])
        if hasattr(self, "button_feedback"):
            self.button_feedback.setGeometry(self.width() - 2 * ERASR_SIZE[0] - 2 * ERASR_OFFSET, ERASR_OFFSET, ERASR_SIZE[0], ERASR_SIZE[1])
        self.update_drawings()
        if hasattr(self, "drawing"):
            self.drawing.setGeometry(0, ERASR_OFFSET * 2 + ERASR_SIZE[1], self.width(), self.height() - DRAWING_Y_OFFSET)
        self.update_drawings()
    ...
    def callback_words_to_write(self, data):
        pts = [(data.data[i * 2], data.data[i * 2 + 1]) for i in range(int(len(data.data) / 2))]
