...
DISTANCE_ROBOT_WRITING_TO_TOP = 80
...
class TactileSurfaceArea(QTableWidget):
    ...
    def __init__(self, parent = None):
        ...
        self.myBrush = QBrush()
        self.myPen = QPen()
        self.upen = QPen() #? pen for user
        self.upen.setWidth(4)
        self.upen.setColor(QColor('#ff0000'))
        self.pixmap = None
        ...
        self.time.start()
        print('[TactileSurfaceArea][init] CALLED')


    def initPixmaps(self):
        print('[TactileSurfaceArea][initPixmaps] CALLED')
        width = 800
        height = 800
        ...
        # self.pixmapHandwriting.fill(QColor(0, 0, 255))

        self.viewport().update()

    def paintEvent(self, event):
        # print(f'[TactileSurfaceArea][paintEvent] CALLED | event = {event}')
        p = QPainter()
    ...
    #? dont have tablet, no need to use this
    def tabletEvent(self, event):
        # print('[TactileSurfaceArea][tabletEvent] called')
        if event.type() == QTabletEvent.TabletPress:
            # print('[TactileSurfaceArea][tabletEvent] event.type() == QTabletEvent.TabletPress')
            ...
            # print('[TactileSurfaceArea][tabletEvent] event.type() == QTabletEvent.TabletRelease')
            if self.deviceDown:
                self.deviceDown = False

        elif event.type() == QTabletEvent.TabletMove:
            # print('[TactileSurfaceArea][tabletEvent] event.type() == QTabletEvent.TabletMove')
            self.lastPoints[1] = self.lastPoints[0]
            self.lastPoints[0] = event.pos()

            if self.deviceDown:
                # print('[TactileSurfaceArea][tabletEvent]    painting on canvas')
    ...
    def mouseMoveEvent(self, event):
        # print('[TactileSurfaceArea][mouseMoveEvent] called')
        painter = QPainter(self.pixmapHandwriting)
        if self.lastPoints[0].isNull():
            # print('[TactileSurfaceArea][mouseMoveEvent][!] self.lastPoints[1].isNul')
            self.lastPoints[0] = event.pos()
        else:
            # self.paintPixmap(painter, event)
            painter.setPen(self.upen)
            painter.drawLine(self.lastPoints[0], event.pos())
            self.viewport().update()
            self.lastPoints[0] = event.pos()

        #? fake pen event
        self.data.append(Data(0, event.x(), event.y(), 30, 30, 0))


    def mouseReleaseEvent(self, e):
        # print('[TactileSurfaceArea][mouseReleaseEvent] called')
        self.lastPoints = [QPoint(), QPoint()]



    def updateBrush(self, event):
        print('[TactileSurfaceArea][updateBrush] called')
    ...
    def paintPixmap(self, painter, event):
        print('[TactileSurfaceArea][paintPixmap] called')
    ...
    def erasePixmap(self):
        print('[TactileSurfaceArea][erasePixmap] called')
    ...
    def eraseRobotTrace(self):
        print('[TactileSurfaceArea][eraseRobotTrace] called')
    ...
    def drawWord(self, path):
        print('[TactileSurfaceArea][drawWord] called')
        ...
        for i in range(len(x)):
            if i > 1:
                if penUp[i] == 0 and penUp[i-1] == 0:
                    ...
                    print('[TactileSurfaceArea][drawWord] painter.drawLine | ', x[i-1], y[i-1], ' | ', x[i-1], y[i-1])
                    self.viewport().update()
        ...
        print('[TactileSurfaceArea][drawWord] drawing done')
        #? advert that drawing is done
        self.signalRobotFinishWriting.emit()

    def drawBoxes(self):
        print('[TactileSurfaceArea][drawBoxes] called')
