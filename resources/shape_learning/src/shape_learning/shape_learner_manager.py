...
# shapeLogger = logging.getLogger('rosout') #? to pipe log to ros handler. still cannot make ros output 
import os.path
...
class ShapeLearnerManager:
    def __init__(self, generateSettingsFunction, shapes_logging_path='shapes.log'):
        ...
        shapeLogger.info('[ShapeLearnerManager] **************** NEW SESSION ***************')
    ...
    def initialiseShapeLearners(self):
        ...
        self.settings_shapeLearners_currentCollection = []
        self.shapeLearnersSeenBefore_currentCollection = []
        print(f'[ShapeLearnerManager][initialiseShapeLearners][ ] self.currentCollection : {self.currentCollection}')
        print(f'[ShapeLearnerManager][initialiseShapeLearners][ ] self.shapesLearnt : {self.shapesLearnt}')
        for i in range(len(self.currentCollection)):
            shapeType = self.currentCollection[i]
            print(f'[ShapeLearnerManager][initialiseShapeLearners]   [ ] shapeType : {shapeType}  |  self.shapesLearnt = {self.shapesLearnt}')

            #? check if shape has been learnt before
            try:
                shapeType_index = self.shapesLearnt.index(shapeType)
                newShape = False
                print(f'[ShapeLearnerManager][initialiseShapeLearners]   [ ] newShape = False. shapeType_index = {shapeType_index}')
            except ValueError:
                print(f'[ShapeLearnerManager][initialiseShapeLearners]   [ ] newShape = True. Dont have shapeType_index')
                newShape = True
            ...
            if newShape:
                ...
                # self.shapeLearners_all[shapeType] = shapeLearner
                ...
                print(f'[ShapeLearnerManager][initialiseShapeLearners]      [ ] self.shapeLearners_all[-1] = {self.shapeLearners_all[-1]} (just appended)')

            else:
                #? still need to append the representation of the letter, otherwise sometimes it raises index out of range
                # self.shapesLearnt.append(shapeType)

                print(f'[ShapeLearnerManager][initialiseShapeLearners]      [ ] self.shapeLearners_all[shapeType_index] = {self.shapeLearners_all[shapeType_index]}')
    ...
    def startNextShapeLearner(self):
        ...
        if self.nextShapeLearnerToBeStarted < len(self.currentCollection):
            ...
            if usePrevParamsWhenShapeReappears and self.shapeLearnersSeenBefore_currentCollection[self.nextShapeLearnerToBeStarted]: #? shape has been seen before
                [path, paramValues] = self.shapeLearners_currentCollection[shape_index].getLearnedShape()
                shapeLogger.info('[ShapeLearnerManager][startNextShapeLearner] %s: continuing learning. Current params: %s. Path: %s' % (shapeType, paramValues.flatten().tolist(), path.flatten().tolist()))
            else:
                [path, paramValues] = self.shapeLearners_currentCollection[shape_index].startLearning()
                shapeLogger.info('[ShapeLearnerManager][startNextShapeLearner] %s: starting learning. Initial params: %s. Path: %s' % (shapeType, paramValues.flatten().tolist(), path.flatten().tolist()))
    ...
    def feedbackManager(self, shapeIndex_messageFor, bestShape_index, noNewShape):
        ...
        if shape_messageFor is None:
            shapeLogger.warning('[ShapeLearnerManager][feedbackManager] Ignoring message because not for valid shape type')
            return -1
        else:
    ...
    def respondToDemonstration(self, shapeIndex_messageFor, shape):
        if shapeIndex_messageFor is None:
            print(f'[ShapeLearnerManager][respondToDemonstration][!] shapeIndex_messageFor = {shapeIndex_messageFor}')
            return
        print(f'[ShapeLearnerManager][respondToDemonstration] shapeIndex_messageFor = {shapeIndex_messageFor}')
        shape_messageFor = self.shapeAtIndexInAllShapesLearnt(shapeIndex_messageFor)
        print(f'[ShapeLearnerManager][respondToDemonstration] shape_messageFor = {shape_messageFor}')
        if shape_messageFor is None:
            shapeLogger.warning('[ShapeLearnerManager][respondToDemonstration] Ignoring demonstration because not for valid shape type')
            return -1
        else:
            ...
            shapeLogger.info('[ShapeLearnerManager][respondToDemonstration] %s: new demonstration.         Params: %s. Path: %s' % (shape_messageFor, params_demo.flatten().tolist(), shape.flatten().tolist()))
            ...
            shapeLogger.info('[ShapeLearnerManager][respondToDemonstration] %s: new generated model.       Params: %s. Path: %s' % (shape_messageFor, newParamValues.flatten().tolist(), newPath.flatten().tolist()))
            return shape
    ...
    def shapeAtIndexInAllShapesLearnt(self, shapeType_index):
        print(f'[ShapeLearnerManager][shapeAtIndexInAllShapesLearnt] self.shapesLearnt = {self.shapesLearnt}')
        shapeType = None
        if shapeType_index < len(self.shapesLearnt):
    ...
    def newCollection(self, collection):
        ...
        for l in collection:
            try:
                ...
                shapeLogger.error('[ShapeLearnerManager][newCollection] No dataset available for letter <%s>. Skipping this letter.' % l)
                continue

            print(f'>> [newCollection] self.currentCollection = {self.currentCollection}  |  l = {l}')
            self.currentCollection += l

        self.nextShapeLearnerToBeStarted = 0

        shapeLogger.info('[ShapeLearnerManager][newCollection] Starting to work on word <%s>' % collection)
    ...
    def resetParameterBounds(self, shapeType_index):
        ...
        shapeLogger.debug('[ShapeLearnerManager][resetParameterBounds] Changing bounds on shape ' + self.shapeAtIndexInCurrentCollection(shapeType_index) + f' from {currentBounds} to {newBounds}')
    ...
    def save_all(self, shapeIndex_messageFor):
        ...
        if (shape_messageFor < 0):
            shapeLogger.warning('[ShapeLearnerManager][save_all] Ignoring demonstration because not for valid shape type')
            return -1
        else:
    ...
    def save_demo(self, shapeIndex_messageFor):
        shape_messageFor = self.shapeAtIndexInAllShapesLearnt(shapeIndex_messageFor)
        if (shape_messageFor < 0):
            shapeLogger.warning('[ShapeLearnerManager][save_demo] Ignoring demonstration because not for valid shape type')
            return -1
        else:
    ...
    def save_params(self, shapeIndex_messageFor):
        shape_messageFor = self.shapeAtIndexInAllShapesLearnt(shapeIndex_messageFor)
        if (shape_messageFor < 0):
            shapeLogger.warning('[ShapeLearnerManager][save_params] Ignoring demonstration because not for valid shape type')
            return -1
        else:
