...
""" custom function to cast all elements in a list to a type"""
def mapfcn(cast_fcn, ar):
    return [cast_fcn(v) for v in ar]
...
class ShapeModeler:
    ...
    def makeDataMatrix(self, filename):
        ...
        # print(f'[ShapeModeler][makeDataMatrix] ref_values = {ref_values}')
        self.refShape = mapfcn(float, ref_values)
        ...
        for i in range(self.numShapesInDataset-1): # -1 because we have already add the first shape (reference)
            ...
            self.dataMat[i+1] = mapfcn(float, values)
        ...
        if self.update_filenames:
            ...
            try:
                with open(filename, 'wb') as f:
                    ...
                    f.write('ref:\n')
                    f.write(' '.join(mapfcn(str, self.dataMat[0])) + '\n')
                    f.write('...\n')
                    for i in range(len(self.dataMat)-1):
                        f.write(' '.join(mapfcn(str, self.dataMat[i+1])))
                        f.write('\n')
            except IOError:
            ...
            for filename in self.update_filenames[1:]:
                ...
                try:
                    with open(filename, 'wb') as f:
                        ...
                        f.write('...\n')
                        for i in range(len(self.dataMat)-1):
                            f.write(' '.join(mapfcn(str, self.demoDataMat[i+1])))
                            f.write('\n')
                except IOError:
                    raise RuntimeError('no writing permission for file'+filename)
    ...
    def normaliseShapeHeight(shape):
        ...
        """
        # print(f'[normaliseShapeHeight] shape = {shape}  |  len(shape) = {len(shape)}')
        numPointsInShape = int(len(shape) / 2)
        print(f'[normaliseShapeHeight] numPointsInShape = {numPointsInShape}')
        x_shape = shape[0:numPointsInShape]
        # print(f'[normaliseShapeHeight] x_shape = {x_shape}')
        y_shape = shape[numPointsInShape:]
